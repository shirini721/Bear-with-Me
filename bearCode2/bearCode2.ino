...
const unsigned long requestInterval = 20*1000;    // delay between requests; 30 seconds
...
Twitter twitter("977484727-7jVoVLVuX2CSS36su3m9Kcf1UY7COBXL1ZCsNbkH");
...
accelXYZSum = analogRead(accelXPin) + analogRead(accelYPin) + analogRead(accelZPin);
  ...
  prevState = currState;
//  Serial.println("Queue Sum: " + queueSum); 
...
Serial.println(timess);
          if (currtime.equals(timess)){ //if there has been no update in the tweets
            ...
            Serial.println("pat");
              ...
              delay(2000);
              digitalWrite(patPin, LOW);
             }else if( first_part.indexOf("the go") != -1 ){
...
void connectToServer() {
  ...
  if (client.connect(server, 80)) {
    ...
    client.println("GET /1/statuses/user_timeline.xml?screen_name=friendly__bear2&count=1 HTTP/1.1");
